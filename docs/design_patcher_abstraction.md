...
>
> ※ 外部出力（OSC による他アプリへの送信など）は現時点では対象外。
...
5. [FX デバイスの設計](#5-fx-デバイスの設計)
6. [デバイス作成の最小手順](#6-デバイス作成の最小手順)
7. [ラック ID の自動取得](#7-ラック-id-の自動取得)
8. [まとめ — 実装方針の推奨](#8-まとめ--実装方針の推奨)
...
| 3 | `brcosa_fx.maxpat` | 輝度/コントラスト FX（アブストラクション使用例） | 10 分 |

### 設計原則（この設計で守っていること）

1. **デバイス間で流れる情報は 3 値以下**（texName + frameIdx + layer）
2. **FX デバイスに JS ファイルは不要** — アブストラクションが全隠蔽
3. **新しい FX はシェーダ差し替えだけで完成** — テンプレートコピー → `jit.gl.pix` 差替え
4. **ラック ID の管理はアブストラクション内部** — FX 作者は気にしなくてよい
